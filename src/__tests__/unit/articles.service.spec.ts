...
describe('ArticlesService', () => {
  ...
  describe('create', () => {
    ...
    it('should default to DRAFT status when none is provided', async () => {
      ...
      expect(mockPrisma.article.create.mock.calls[0][0].data.status).toBe(
        'DRAFT',
      );
    });
    ...
    it('should use connectOrCreate for each tag so existing tags are reused', async () => {
      ...
      expect(data.tags.connectOrCreate[0]).toEqual({
        where: { name: 'a' },
        create: { name: 'a' },
      });
    });
    ...
    it('should set authorId to null when not provided', async () => {
      ...
      expect(
        mockPrisma.article.create.mock.calls[0][0].data.authorId,
      ).toBeNull();
    });
  });
  ...
  describe('update', () => {
    it('should transition status from draft to published', async () => {
      ...
      const result = await service.update('art-1', {
        status: ArticleStatus.PUBLISHED,
      });

      expect(result.status).toBe('published');
      expect(mockPrisma.article.update.mock.calls[0][0].data.status).toBe(
        'PUBLISHED',
      );
    });
    ...
    it('should pass undefined to Prisma when an invalid status is provided', async () => {
      ...
      expect(
        mockPrisma.article.update.mock.calls[0][0].data.status,
      ).toBeUndefined();
    });
